...
void entornoPintarPieza(TipoZona zona, int ancho, int alto, int color) {
	...
	// verticales
	for (i = 1; i < (ancho); i++) {
		al_draw_line( fila + i * tam,col + 0 * tam, fila + i * tam,col + alto * tam, makecolor2(COLOR_LINEAS), GROSOR_LINEAS);
	}
	//horizontales
	for (i = 1; i < (alto); i++) {
		al_draw_line( fila + 0 * tam, col + i * tam , fila + ancho * tam, col + i * tam, makecolor2(COLOR_LINEAS),GROSOR_LINEAS);
	}
	al_flip_display();
}
